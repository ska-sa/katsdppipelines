...
class SimData(katdal.H5DataV2):
    ...
    def h5toSPEAD(self,ts,l0_endpoint,spead_rate=1e9,max_scans=None):
        ...
        for scan_ind, scan_state, target in self.scans(): 
            ...
            ts.add('{0}_target'.format(self.ref_ant,),target.description,ts=self.timestamps[0]-random()*0.1)
            ts.add('{0}_activity'.format(self.ref_ant,),scan_state,ts=self.timestamps[0]-random()*0.1)
