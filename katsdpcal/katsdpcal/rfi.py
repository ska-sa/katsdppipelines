...
def getbackground_2dspline(data, spike_width):
    ...
    """From a 2-d data array determine a background iteratively by fitting a
    spline and removing data more than a few sigma from the spline.
    """
...
def getbackground_median(data, spike_width_freq=5, spike_width_time=5):
    ...
    data : :class:`np.ndarray`
        a 2d array (time, frequency) of floats
    spike_width_freq : int
        the median filter width in frequency
    spike_width_time : int
        the median filter width in time (ignored for 1d input data)
    """

    if len(data.shape) == 1:
        kernel = spike_width_freq
    else:
        kernel = (spike_width_time, spike_width_freq)
    return ndimage.filters.median_filter(data, size=kernel)


def detect_spikes_sumthreshold(data, blarray=None, spike_width=5, outlier_sigma=11.0,
                               window_size_auto=[1, 3, 5], window_size_cross=[2, 4, 8]):
    """
    ...
    for bl_index in range(data.shape[-1]):
        ...
        if blarray is None or bl_name[0][:-1] == bl_name[1][:-1]:
            # Auto-Correlation.
            # filtered_data = np.asarray(signal.medfilt(this_data_buffer, kernel_size),
            #                            this_data_buffer.dtype)
        ...
        # Assuming normally distributed deviations, this is a robust estimator
        # of the standard deviation
        estm_stdev = 1.4826 * med_abs_dev

        # Identify initial outliers (again based on normal assumption), and
        ...
        for window in window_bl:
            ...
            # Flags are padded with zeros to ensure the flag array (derived
            # from the convolved data) has the same dimension as the input
            # data.
...
def detect_spikes_mad(data, blarray=None, spike_width=20, outlier_sigma=3):
    ...
    for bl_index in range(data.shape[-1]):
        ...
        # filtered_data = np.asarray(signal.medfilt(spectral_data, kernel_size),
        #                            spectral_data.dtype)
...
def rolling_window(a, window, axis=-1, pad=False, mode='reflect', **kargs):
    ...
    window : int
        window size
    axis : int, optional
        axis to move the window over; default is the last axis.
    pad : bool, optional
        Pad the array to the original size
    mode : str or function, optional
        Padding mode passed to :func:`numpy.pad`)
    **kwargs : dict
        Passed to :func:`numpy.pad`

    Returns
    -------
    array
        an array with shape ``np.array(a.shape + (window,))`` and the rolled
        data on the last axis

    Examples
    --------
    >>> import numpy as np
    >>> data = np.random.normal(
    ...     loc=1, scale=np.sin(5*np.pi*np.arange(10000).astype(float)/10000.)+1.1, size=10000)
    ...
    return np.lib.stride_tricks.as_strided(
        a1, shape=shape, strides=strides).swapaxes(-2, axis)  # Move original axis to
...
def detect_spikes_orig(data, axis=0, spike_width=2, outlier_sigma=11.0):
    ...
    .. todo::
       ...
       Make this more like a Hampel filter by making MAD time-variable too.
    ...
    # med_abs_dev = signal.medfilt(abs_dev, kernel)
    # Assuming normally distributed deviations, this is a robust estimator of
