...
def calc_uvdist(ts, target, freq, times):
    ...
    kat_target = katpoint.Target(target)
    u, v, w = calprocs.calc_uvw_wave(kat_target, times, ts['cal_bls_lookup'][cross_idx],
...
def make_cal_report(ts, stream_name, report_path, av_corr, project_name=None, st=None, et=None):
    ...
    chanav = av_corr['vis'].shape[-3] // 8
    chanidx = ts[stream_name + '_n_chans']
    
    total_chanav = ts['cal_channel_freqs'].shape[0] // 8
            ...
            for ti in range(len(times)):
                ...
                for idx in range(0, vals[ti].shape[-1], ant_chunks):
                    ...
                    insert_fig(report_path, cal_rst, plot,
                               name='{0}_ti_{1}_{2}'.format(cal, ti, idx))
    ...
    if len(av_corr['targets']) > 0:
        ...
        # separate gain-calibrated calibrators from non gain-calibrated targets 
        nogain, gain, target = [], [], []
        for cal in unique_targets:
    ...
    cal_rst.write_heading_2('Corrected Amp vs Frequency, gain calibrators')
    cal_rst.writeln()
    ...
    cal_rst.writeln()
    cal_rst.write_heading_3('All baselines')
    cal_rst.writeln()
    ...
    cal_rst.writeln()
    cal_rst.write_heading_2('Amp vs Frequency, all target fields')
    cal_rst.writeln()
    cal_rst.write_heading_3('All baselines, averaged per antenna')
    cal_rst.writeln()
    ...
    cal_rst.write_heading_2('Amp vs UV distance, all target fields')
    cal_rst.writeln()
    cal_rst.write_heading_3('All baselines')
    cal_rst.writeln()
    for cal in target:
